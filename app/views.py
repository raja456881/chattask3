...
def creategroup(request):
    if request.method=="POST":
        ...
        submit=request.POST.getlist('select')
        group1=Group.objects.get_or_create(name=group)
        my_group = Group.objects.get(name=group)
        print(my_group)
        list=[]
        for i in submit:
            print(i)
            user1=User.objects.get(pk=i)
            my_group.user_set.add(user1)
            list.append(user1.username)
        return redirect('group', group1=my_group)
    user=User.objects.all()
    return render(request, 'esss/group.html', {"user": user})

def groupchat(request, group1):
    try:
        user1 = request.user
        username1 = User.objects.get(username=request.user)
        id = username1.id
        return render(request, 'esss/groupchat.html', {'current_user_id': id})
    except:
        return render(request, 'esss/groupchat.html')
@method_decorator(login_required(login_url='/home/'), name='dispatch')
class Groupdetails(ListAPIView):
...
class JoinGroup(View):
    def get(self, request):
        return render(request, 'esss/Groupdetails.html')
    def post(self, request):
        try:
            ...
            return redirect('home')
              ...
              return HttpResponse("This name Group is not exists ")
@method_decorator(login_required(login_url='/home/'), name='dispatch')
class  LeaveGroup(View):
    def get(self, request):
        return render(request, 'esss/Groupleave.html')
    def post(self, request):
        group_id = request.POST['group_id']
